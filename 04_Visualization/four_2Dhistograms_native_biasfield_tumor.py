...
def get_scatterplot_native_biasfield(mri_fname:str, array_mri:np.array,array_biasfield:np.array,brain_seg_array:np.array, tumor_bin_array:np.array, display = False, save = False, ax = None):
    # Try remove 0
    mask_tumor = array_mri * tumor_bin_array
    mask = mask_tumor > 0  # Only include voxels with non-zero native intensities
...
def compute_all_scatterplots_mri_type(new_dir_path, patient_dir_name_nifti, mri_type:str, patient_number, display=False, save=False):
    ...
    # Tumor segmentation
    tumor_bin_array_path = os.path.join(array_dir_path, f'{patient_dir_name}_tumor_binary_array.npy')
    tumor_bin_array = np.load(tumor_bin_array_path).astype(np.float32)
    ...
    get_scatterplot_native_biasfield(bias_n4_brain_nameplot, native_image_array, bias_n4_brain_array,brain_seg_array, tumor_bin_array, display=display, save=save,ax=axs[0, 0])
    get_scatterplot_native_biasfield(bias_n4_healthy_nameplot, native_image_array, bias_n4_healthy_array, brain_seg_array, tumor_bin_array, display=display,save=save, ax=axs[0, 1])
    get_scatterplot_native_biasfield(bias_n4_brain_healthy_nameplot, native_image_array, bias_n4_brain_healthy_array, brain_seg_array, tumor_bin_array, display=display, save=save, ax=axs[1, 0])
    get_scatterplot_native_biasfield(bias_n4_healthy_brain_nameplot, native_image_array, bias_n4_healthy_brain_array, brain_seg_array, tumor_bin_array, display=display, save=save, ax=axs[1, 1])
