...
def get_scatterplot_native_biasfield(mri_fname:str, array_mri:np.array, array_biasfield:np.array,brain_seg_array:np.array, tumor_mask_array:np.array, display = False, save = False, ax = None):
    if ax is None:
        fig, ax = plt.subplots()
...
def compute_all_scatterplots_mri_type(new_dir_path, patient_dir_name_nifti, mri_type:str, patient_number, display=False, save=False):
    ...
    get_scatterplot_native_biasfield(bias_n4_brain_nameplot, native_image_array, bias_n4_brain_array,brain_seg_array,tumor_binary_array, display=display, save=save,ax=axs[0, 0])
    get_scatterplot_native_biasfield(bias_n4_healthy_nameplot, native_image_array, bias_n4_healthy_array, brain_seg_array, tumor_binary_array, display=display,save=save, ax=axs[0, 1])
    get_scatterplot_native_biasfield(bias_n4_brain_healthy_nameplot, native_image_array, bias_n4_brain_healthy_array, brain_seg_array, tumor_binary_array, display=display, save=save, ax=axs[1, 0])
    get_scatterplot_native_biasfield(bias_n4_healthy_brain_nameplot, native_image_array, bias_n4_healthy_brain_array, brain_seg_array, tumor_binary_array, display=display, save=save, ax=axs[1, 1])
