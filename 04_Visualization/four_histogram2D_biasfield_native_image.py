...
def calculate_histogram2D_biasfield_native(mri_fname, native_mri_array:np.array,biasfield_array:np.array, bins = 655, display = False, save = False, ax = None):
    # Checking sizes
    ...
    ax.set_title(f'Histogram 2D of Intensities (Rescaled): {mri_fname}', color='black')
...
def compute_all_histogram2D_mri_type(new_dir_path, patient_dir_name_nifti, mri_type:str, patient_number, bins=655, display=False, save=False):
    ...
    calculate_histogram2D_biasfield_native(bias_n4_brain_nameplot, bias_n4_brain_array, native_image_array, bins=bins, display=display, save=save,ax=axs[0, 0])
    calculate_histogram2D_biasfield_native(bias_n4_healthy_nameplot, bias_n4_healthy_array, native_image_array, bins=bins, display=display,save=save, ax=axs[0, 1])
    calculate_histogram2D_biasfield_native(bias_n4_brain_healthy_nameplot, bias_n4_brain_healthy_array, native_image_array,bins=bins, display=display, save=save, ax=axs[1, 0])
    calculate_histogram2D_biasfield_native(bias_n4_healthy_brain_nameplot, bias_n4_healthy_brain_array, native_image_array, bins=bins, display=display, save=save, ax=axs[1, 1])
...
def continue_main_for_new_patient(NEW_DIR, folder_name, mri_type, patient_number, display=True, save=False):
    ...
    while user_ans == 'Y':
        ...
        compute_all_histogram2D_mri_type(NEW_DIR, f'UCSF-PDGM-{patient_number}_nifti', mri_type, patient_number,
